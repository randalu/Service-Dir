    ...
    if (!$name) {
        ...
        if ($action === 'create') {
            ...
            if ($stmt->fetch()) {
                ...
                $stmt->execute([$name]);
                header('Location: manage_categories.php?msg=Category added');
                exit;
            }
            ...
            if ($stmt->fetch()) {
                ...
                $stmt->execute([$name, $catId]);
                header('Location: manage_categories.php?msg=Category updated');
                exit;
            }
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'delete' && $catId) {
...
<div class="dashboard">
  <div class="sidebar">
    <div class="brand"><span>Admin Panel</span><small>Categories</small></div>
    <div class="nav">
      <a href="dashboard.php"><i class="bi bi-speedometer2"></i><span>Dashboard</span></a>
      <a href="manage_ads.php"><i class="bi bi-megaphone"></i><span>Manage Ads</span></a>
      <a href="manage_categories.php" class="active"><i class="bi bi-folder"></i><span>Categories</span></a>
  ...
  <div class="main">
    <div class="d-flex justify-content-between align-items-center mb-4">
      ...
      <button class="btn btn-primary" style="background:var(--forest);border:none;border-radius:var(--radius);font-weight:600;font-size:0.85rem;padding:0.5rem 1.2rem" data-bs-toggle="modal" data-bs-target="#createCategoryModal">+ Add Category</button>
    </div>
    ...
    <div class="table-responsive">
      <table class="table table-modern">
        <thead>
          <tr><th>ID</th><th>Name</th><th style="width:180px">Actions</th></tr>
        </thead>
        <tbody>
          <?php if (!$categories): ?>
            ...
            <?php foreach ($categories as $cat): ?>
              <tr>
                ...
                <td>
                  ...
                  <form method="POST" action="?action=delete&id=<?= $cat['id'] ?>" class="d-inline" onsubmit="return confirm('Delete this category?');">
                    <?= csrfField() ?>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <form method="POST" action="?action=create" class="modal-content" style="border:none;border-radius:var(--radius-lg)">
      ...
      <div class="modal-footer" style="border-top:1px solid var(--gray-border)">
        <button type="submit" class="btn btn-primary" style="background:var(--forest);border:none;border-radius:var(--radius);font-weight:600">Add Category</button>
...
<div class="modal fade" id="editCategoryModal" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered">
    <form method="POST" id="editCategoryForm" class="modal-content" style="border:none;border-radius:var(--radius-lg)">
      <?= csrfField() ?>
      <div class="modal-header" style="border-bottom:1px solid var(--gray-border)">
      ...
      <div class="modal-body">
        ...
        <label class="form-label fw-semibold small">Category Name</label>
        <input type="text" name="name" id="editCatName" class="form-control" style="border:1.5px solid var(--gray-border);border-radius:var(--radius);padding:0.6rem 1rem" required>
      </div>
      <div class="modal-footer" style="border-top:1px solid var(--gray-border)">
        <button type="submit" class="btn btn-primary" style="background:var(--forest);border:none;border-radius:var(--radius);font-weight:600">Update</button>
        <button type="button" class="btn btn-secondary" style="border-radius:var(--radius)" data-bs-dismiss="modal">Cancel</button>
      </div>
    </form>
  </div>
...
document.getElementById('editCategoryModal')?.addEventListener('show.bs.modal', event => {
  const cat = JSON.parse(event.relatedTarget.getAttribute('data-cat'));
  document.getElementById('editCatId').value = cat.id;
  document.getElementById('editCatName').value = cat.name;
  document.getElementById('editCategoryForm').action = 'manage_categories.php?action=edit&id=' + cat.id;
});
</script>
</body>
</html>